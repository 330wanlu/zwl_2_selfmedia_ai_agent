...
- 封面要有视觉冲击力，内页图信息密度适中（一眼能看懂在讲什么）
- 整组风格统一：明亮、干净、小红书美妆风
- 画面以产品/场景/氛围为主；中文只允许极短标题氛围字（≤6 字），且要清晰可读
...
2. image_prompt：给 AI 文生图模型的中文画面描述，要求：
   ...
   - 封面（sequence=1）：视觉冲击力强，构图简洁；可含一句 ≤6 字中文标题氛围
   - 内容页：信息感强，可搭配美妆道具（化妆刷、镜子、花瓣等）但不杂乱
   - 整组 4 张图色调和光线保持一致
   - 禁止满屏大段文字、禁止英文乱码
   - 严禁出现真实品牌 logo、可识别商标、明星或网红面孔；用「无品牌标识的瓶身/管身」描述
